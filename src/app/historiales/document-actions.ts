...
export async function listMedicalDocuments(filter:DocumentFilter):Promise<{documents:MedicalDocument[];more:boolean}> {
  ...
  if(error) throw new Error('No se pudieron cargar los formatos. Verifica la conexión y que esté aplicada la actualización 004 en Supabase.');
  return data;
}
export async function saveMedicalDocument(input:unknown):Promise<{document?:MedicalDocument;error?:string}> {
  try {
    ...
    const {data,error}=await db.rpc('medical_document_save',{session_token:token,payload});
    if(error) return {error:error.code==='40001'?'Este borrador cambió en otra ventana. Conserva tus cambios y vuelve a abrir la versión guardada.':'No se confirmó el guardado. Revisa los campos, tu conexión y la actualización 004. Conserva el formulario e intenta nuevamente.'};
