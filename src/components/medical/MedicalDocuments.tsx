...
function WorkerPicker({search,onSelect,selected}:{search:WorkerSearch;onSelect:(w:MedicalWorker)=>void;selected?:string}){
  ...
  return <div className="rounded-xl border border-blue-100 bg-blue-50/40 p-3 space-y-2"><label className="text-xs font-bold text-[#002f6c]">Vincular con el padrón de trabajadores</label>{selected&&<p className="text-sm font-bold">{selected}</p>}<div className="flex gap-2"><input aria-label="Buscar trabajador para formato" className="medical-input !mt-0" placeholder="Nombre o CI" value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();e.currentTarget.parentElement?.querySelector('button')?.click();}}}/><button type="button" disabled={busy} className="medical-primary" onClick={async()=>{setBusy(true);setError('');try{const result=await search(query);setWorkers(result);if(!result.length)setError('No hay coincidencias.');}catch{setError('No se pudo buscar. Revisa tu sesión y conexión.');}finally{setBusy(false);}}}><Search size={15}/>{busy?'Buscando…':'Buscar'}</button></div>{error&&<p role="alert" className="text-red-700 text-xs">{error}</p>}{workers.length>0&&<div className="max-h-52 overflow-y-auto divide-y bg-white border rounded-lg">{workers.map(w=><button type="button" className="block text-left w-full p-3 hover:bg-blue-50 text-sm" key={w.id} onClick={()=>{onSelect(w);setWorkers([]);setQuery('');}}>{w.full_name} <span className="text-slate-500">· CI {w.ci}</span></button>)}</div>}</div>;
}
...
export default function MedicalDocuments({doctor,search,api=documentsApi,initialWorker,initialTemplate,initialDocument,onDirty,onBusy}:{doctor:string;search:WorkerSearch;api?:DocumentsApi;initialWorker?:MedicalWorker;initialTemplate?:string;initialDocument?:MedicalDocument;onDirty:(v:boolean)=>void;onBusy:(v:boolean)=>void}){
  ...
  async function save(finalize:boolean){
    ...
    try{
      ...
      setNotice(finalize?(doc.template_id==='historia'?'Historia finalizada. Se incorporó al Registro de Consultas.':'Documento finalizado. Ya puedes imprimirlo.'):'Borrador guardado. Puedes continuar después.');
      setConfirmFinal(false);
    }catch(e){setError(e instanceof Error?e.message:'No se confirmó el guardado.');setConfirmFinal(false);}
  ...
  return <section className="medical-documents space-y-5">
    ...
    {error&&<div role="alert" className="bg-red-50 border border-red-200 text-red-800 p-4 rounded-xl">{error}</div>}{notice&&<div role="status" className="bg-green-50 border border-green-200 text-green-800 p-4 rounded-xl">{notice}</div>}
    ...
    {view==='history'&&<>
      ...
      <WorkerPicker search={search} selected={workerName} onSelect={w=>{setFilter({...filter,worker:w.id});setWorkerName(w.full_name);}}/>{filter.worker&&<button className="text-blue-700 text-xs underline" onClick={()=>{setFilter({...filter,worker:undefined});setWorkerName('');}}>Quitar filtro de trabajador</button>}
    ...
    {view==='editor'&&doc&&template&&<>
      ...
      <div className="sticky bottom-0 bg-white/95 border rounded-xl p-4 flex flex-wrap gap-3 items-center shadow-lg">
        ...
        {doc.status==='draft'&&<><button disabled={busy||!!rowEditor} className="medical-primary" onClick={()=>void save(false)}><Save size={16}/>{busy?'Guardando…':'Guardar borrador'}</button><button disabled={busy||!!rowEditor} className="border border-blue-800 text-blue-900 rounded-xl px-4 py-2 font-bold text-sm" onClick={()=>{try{validateDocument(payload(true));setConfirmFinal(true);setError('');}catch(e){setError(e instanceof Error?e.message:'Revisa los campos.');}}}><Check size={16} className="inline mr-1"/> Revisar y finalizar</button></>}
